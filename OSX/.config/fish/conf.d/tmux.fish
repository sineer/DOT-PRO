...
# Use a file-based flag to track autostart across different login sessions
set -l autostart_flag_file "$HOME/.cache/fish_tmux_autostarted"
# Check if flag file exists AND if tmux is still running
if test -e "$autostart_flag_file"
    # If tmux is not running anymore, clean up the flag file
    if not command tmux list-sessions &>/dev/null
        rm -f "$autostart_flag_file"
        set -gx fish_tmux_autostarted false
    else
...
function _fish_tmux_plugin_run
    ...
    # try to connect to an existing session only if autoconnect is true
    if test "$fish_tmux_autoconnect" = true
        ...
        # autoconnect is false, so we'll create a new session
        set attach_status 1
    end

    # if autoconnect is false or attach failed, create new session
...
# function to reset the autostart flag
function _fish_tmux_reset_autostart
    rm -f "$HOME/.cache/fish_tmux_autostarted"
    set -gx fish_tmux_autostarted false
    echo "Fish tmux autostart flag has been reset"
end

function _fish_tmux_plugin_run_autostart --on-variable fish_tmux_autostart --on-variable fish_tmux_autostart_once
    ...
    test "$TERMINAL_EMULATOR" != 'JetBrains-JediTerm'
        # If autostart_once is not set, wait for config to set it
        if not set -q fish_tmux_autostart_once
            # Variable not set yet, skip for now
            return
